...
from excel_import import parse_courses_xlsx, import_courses


class CoursesUploadPage(QWidget):
    def __init__(self, user: dict):
        ...
        self.setStyleSheet("""
            ...
            QFrame#DropZone {
                background:#F9FAFB; border:2px dashed #C7CDD4; border-radius:14px;
            }
        ...
        # Başlık
        t = QLabel("Ders Listesi Yükleme"); t.setObjectName("Title")
        s = QLabel("Excel parser ile ders yükleme."); s.setObjectName("Subtitle")
        ...
        self.drop.setMinimumWidth(640); self.drop.setMinimumHeight(160)
        ...
        pick_btn = QPushButton("Dosya Seç…"); pick_btn.setObjectName("Ghost"); pick_btn.clicked.connect(self._pick_file)
        dz.addWidget(self.dz_title); dz.addWidget(self.sel_lbl); dz.addWidget(pick_btn, 0, Qt.AlignmentFlag.AlignCenter)

        cz = QHBoxLayout()
        cz.addStretch(1); cz.addWidget(self.drop); cz.addStretch(1)
        center.addLayout(cz)

        center.addStretch(1)
        root.addLayout(center)

        # Alt satır: buton + progress
        row = QHBoxLayout()
        row.addStretch(1)
        self.upload_btn = QPushButton("Excel'den Ders Listesi Yükle"); self.upload_btn.setObjectName("Primary")
    ...
    def _pick_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Ders Excel'i Seç", "", "Excel (*.xlsx *.xls)")
        if path: self._set_file(path)

    def _busy(self, on: bool):
        self.prg.setVisible(on); self.upload_btn.setEnabled(not on); self.drop.setEnabled(not on)
    ...
    def _start_import(self, path: str, department_id: int):
        try:
            self._busy(True)
            df = parse_courses_xlsx(path)
            if df.empty:
                QMessageBox.information(self, "Ders Yükleme", "Excel'de ders satırı bulunamadı.")
                return

            ins, upd, unresolved = import_courses(df, department_id)

            self.banner_lbl.setText(
                f"Tamamlandı • Yeni ders: {ins} • Güncellenen: {upd} • Eşleşmeyen öğretim elemanı: {unresolved}"
            )
            self.banner.setVisible(True)
            QMessageBox.information(
                self, "Ders Yükleme",
                f"Tamamlandı.\nYeni ders: {ins}\nGüncellenen: {upd}\n"
                f"Eşleşmeyen öğretim elemanı: {unresolved} (eşleşmeyenlerde InstructorID=NULL)"
            )
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Ders listesi yüklenemedi:\n{e}")
        finally:
            self._busy(False)
